...
3.  **Robust Cleanup**: Before building, the script MUST stop and remove any container with the same name. This handles both running and stopped containers. Use this exact logic:
...
"""
    ...
    # Forcefully remove all backticks to prevent shell syntax errors from LLM formatting.
    script = script.replace('`', '')

    # Robustly clean the script to remove any text before the shebang.
    shebang = "#!/bin/bash"
    if shebang in script:
        script = script[script.find(shebang):]
    elif script.startswith("```"): # Fallback for markdown code fences
